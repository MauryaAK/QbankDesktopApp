...
type QuestionStatus = "none" | "ok" | "not_ok";

const QuestionStatusToggle = ({
  value,
  onChange,
}: {
  value: QuestionStatus;
  onChange: (next: QuestionStatus) => void;
}) => {
  const nextState = () => {
    if (value === "none") return "not_ok";
    if (value === "not_ok") return "ok";
    return "none";
  };
  ...
  const styles = {
    ...
    not_ok: "bg-red-100 text-red-700 border-red-400",
  };
  ...
  return (
    <button
      ...
      className={`px-3 py-[2px] text-xs font-semibold rounded-full border transition-colors ${styles[value]}`}
    >
      {labels[value]}
    </button>
  );
