...
import { generateAtaGroupColumn, generateExamPaper } from "../../utils/tableColumns";
...
const GenerateExamPaper = () => {
    ...
    const getAtaForExamGenerationQuery: any = useQuery({
        queryKey: ["getAtaForExamGeneration", userId],
        queryFn: getAtaForExamGeneration,
        enabled: !!userId,
    });
    ...
    const isLoading =
        getAtaForExamGenerationQuery.isLoading ||
        mutation.isPending;
    ...
    const allRows = useMemo(() => {
        const rows = getAtaForExamGenerationQuery?.data?.atas ?? [];
        return withRowId(rows);
    }, [getAtaForExamGenerationQuery?.data?.atas]);
    ...
    const filterFieldsWithOptions: any = useMemo(() => {
        return GENERATE_EXAM_PAPER.map((field) => ({
            ...field,
            options:
        ...
        <div className="h-screen flex flex-col ">
            ...
            <div className="mx-20 mt-5">
                <div>
                    <FilterSectionForExam
                        ...
                        key={filterKey}
                        label="Generate Exam Paper/Batch"
                        ...
                        <div className="flex items-center justify-between shrink-0 mt-[1%] ">
                            ...
                            <div className="flex items-center w-[17%] h-8 rounded-xl bg-[#C3BFBF] border border-red-200 shadow-sm px-2 mt-6 mr-0">
                                <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                                    <img src={searchIcon} className="w-8 h-8 mr-16" alt="" />
                                </div>
            ...
            <Footer
                buttons={[
                    ...
                    { label: "Generate Hall Ticket", onClick: handleSubmit },
                ]}
            />
        </div>
    );
